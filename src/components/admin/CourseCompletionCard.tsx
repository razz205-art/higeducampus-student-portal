...
import type { CourseCompletionRow } from "@/types/analytics";

export default function CourseCompletionCard({ rows }: { rows: CourseCompletionRow[] }) {
  return (
    <DashboardCard title="Course Completion" icon={CheckCircle2} bodyClassName="p-0">
      {rows.length === 0 ? (
            ...
            <tbody className="divide-ink-900/8 divide-y">
              {rows.map((r) => (
                <tr key={r.courseId}>
                  ...
                  <td className="px-5 py-3 text-ink-900/70">{r.enrolledCount}</td>
                  <td className="px-5 py-3 text-ink-900/70">{r.completedCount}</td>
                  <td className="px-5 py-3">
