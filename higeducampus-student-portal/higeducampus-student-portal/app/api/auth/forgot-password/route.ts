...
import { generateResetToken, RESET_TOKEN_TTL_MINUTES } from "@/lib/tokens";
...
export async function POST(req: NextRequest) {
  ...
  // Rate limit by IP AND by email (parsed below) to blunt both spray attacks
  // and targeted enumeration against a single account.
  ...
  if (!emailLimit.success) {
    // Same generic response — don't reveal that the limit was hit for this email.
    return NextResponse.json(GENERIC_RESPONSE, { status: 200 });
  }
